...
@activity.defn(name="train-table-model")
...
@activity.defn(name="sample-table-model")
async def sample_model(input: SampleModelInput):
  ...
  samples['created_at'] = pd.to_datetime(samples['created_at'], unit='ns')
  samples['updated_at'] = pd.to_datetime(samples['updated_at'], unit='ns')
